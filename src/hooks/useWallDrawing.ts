...
export const useWallDrawing = ({
  ...
  const continueDrawing = useCallback((e: any) => {
    if (!fabricCanvasRef.current || !isDrawing || !currentLine || tool !== DrawingMode.WALL) return;
    ...
    canvas.renderAll();
  }, [fabricCanvasRef, isDrawing, currentLine, tool]);
  
  // End drawing the wall
  const endDrawing = useCallback(() => {
    if (!fabricCanvasRef.current || !isDrawing || !currentLine) return;
    
    const canvas = fabricCanvasRef.current;
    
    // Straighten the line if needed
    straightenLine(currentLine);
    
    // Finalize the line
    canvas.renderAll();
    
    // Fire object:added event for history tracking
    canvas.fire('object:added', { target: currentLine });
  ...
  }, [fabricCanvasRef, isDrawing, currentLine, straightenLine]);
